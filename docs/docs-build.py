...
def build_docs(ref):
    ...
    with open(source_dir + os.path.sep + 'version.py', 'w',
              encoding='utf-8') as f:
    ...
    call_cmd(
        'LATEXOPTS="-interaction=batchmode" sphinx-build -M latexpdf %s %s' %
        (source_dir, dest_dir))
    ...
    call_cmd('mv %s/latex/%s.pdf %s/%s-%s.pdf ; rm -Rf %s/latex/' %
             (dest_dir, project_name, dest_dir, project_name, ref, dest_dir))
    call_cmd('mv %s/html/* %s/ ; rm -Rf %s/html/ ; rm -Rf %s/doctrees/' %
...
if not git_current:
    raise RuntimeError(
        'Could not determine current tag/branch name, pass by first argument')
...
if not git_default:
    raise RuntimeError(
        'Could not determine default branch name, pass by second argument')

# clean up branch names, remove up to last "origin/"
git_branches = [
    re.sub(r'^.*origin/', '', b) for b in git_branches if 'origin/' in b
]
...
git_doc_branches = [
    branch for branch in git_branches
    if sphinx_config_valid('origin/' + branch)
]
...
existing_docs = [
    e for e in os.listdir(build_dir)
    if os.path.isdir(build_dir + os.path.sep + e)
]
print('Existing docs:')
print(existing_docs)

all_buildable_docs = set(git_doc_tags).union(git_doc_branches).union(
    [git_current])
...
with open(build_dir + os.path.sep + 'versions.json', 'w',
          encoding='utf-8') as f:
    json.dump(
        {
            'tags': git_doc_tags,
...
with open(build_dir + os.path.sep + 'index.html', 'w', encoding='utf-8') as f:
    ...
    f.write('<body>\n')
    f.write(
        '<p>Please wait while we redirect you to our <a href="%s/index.html">latest documentation</a>.</p>\n'
        % git_default)
    f.write('</body>\n')
    f.write('</html>\n')
